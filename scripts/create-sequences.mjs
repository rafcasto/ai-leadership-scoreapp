// Builds the 4 evergreen drip SEQUENCES (and their emails) in Kit via the API.
// Idempotent: reuses a sequence by name and skips emails whose subject exists.
//   node scripts/create-sequences.mjs
//
// NOTE: Kit visual AUTOMATIONS still must be built in the UI (no API). After
// running this, wire the 2 automations from kit-build-guide.md.
...
/* ---- email HTML helpers (match the broadcasts) ---- */
const RED = "#c2001f", INK = "#191c27";
const wrap = (parts) => `<div style="font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:1.55;color:${INK}">${parts.join("")}</div>`;
const p = (s) => `<p style="margin:0 0 16px">${s}</p>`;
const ul = (items) => `<ul style="margin:0 0 16px;padding-left:20px">${items.map((i) => `<li style="margin:0 0 8px">${i}</li>`).join("")}</ul>`;
const btn = (label, href) => `<p style="margin:24px 0"><a href="${href}" style="background:${RED};color:#fff;text-decoration:none;font-weight:bold;padding:13px 24px;border-radius:10px;display:inline-block">${label}</a></p>`;
const scores = () => `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 16px">
  ...
  <tr><td style="padding:2px 0;color:#3a3f4d">Leadership</td><td style="padding:2px 0 2px 18px;font-weight:bold">{{ subscriber.leadership_score }}/5</td></tr></table>`;
const sig = (full = false) => p(`— Laurent${full ? " Simon, AI Dojo" : ""}`);

const SEQUENCES = [
  {
    name: "Scorecard – Results",
    emails: [
      { delay: 0, subject: "Your AI Readiness Score, {{ subscriber.first_name }}",
        ...
        content: wrap([
          ...
          p("Your full personalized report (benchmark chart + recommendations for each area):"),
          ...
          p(`Want to talk through your results? Book a free 20-minute AI Readiness debrief: <a href="[LINK]" style="color:${RED}">[LINK]</a>`),
          sig(true),
        ]) },
    ],
  },
  {
    name: "Scorecard – Abandon",
    emails: [
      { delay: 0, subject: "You're one step from your AI Readiness Score",
      ...
      { delay: 2, subject: "Still curious how AI-ready your leaders are?",
        preview: "One stat most leadership teams underestimate.",
        content: wrap([
          p("Hi {{ subscriber.first_name }},"),
          p("A quick thought before you go: the gap between companies isn't access to AI — it's whether leaders know how to put it to work. That gap is widening fastest at the leadership level."),
  ...
  {
    name: "Track A – Build the Foundations",
    emails: [
      { delay: 2, subject: "The fastest way to raise your AI readiness",
        ...
        content: wrap([
          ...
          ul(["Name one owner and one 30-day goal for {{ subscriber.lowest_category }}.",
          ...
          p(`Want help prioritizing? <a href="[LINK]" style="color:${RED}">Book a free debrief</a>.`),
          sig(),
        ]) },
      { delay: 3, subject: "Why AI readiness stalls (and how to unstick it)",
      ...
      { delay: 4, subject: "Give your leaders permission to test and learn",
        ...
        content: wrap([
          ...
          ul(["A sandbox: an approved tool + a few low-risk use cases to start.",
              "A use-case shortlist so people aren't staring at a blank box.",
              'A "responsible use" one-pager so the boundaries are obvious.']),
          btn("Book a debrief →", "[LINK]"),
          sig(),
        ]) },
      { delay: 5, subject: "Want a second pair of eyes on your results?",
  ...
  {
    name: "Track B – Scale the Advantage",
    emails: [
      { delay: 2, subject: "You're past the starting line — now scale it",
      ...
      { delay: 3, subject: "The metric most AI-ready teams still miss",
      ...
      { delay: 4, subject: "Turn your AI edge into an operating standard",
        ...
        content: wrap([
          ...
          ul(["Standardize the prompts that work into shared, named templates.",
              "Codify responsible use so it's a default, not a debate.",
              "Name an owner per function so momentum has a home."]),
          btn("Book a strategy call →", "[LINK]"),
          sig(),
        ]) },
      { delay: 5, subject: "A strategy session on scaling your AI advantage",
...
async function ensureSequence(name) {
  ...
  const found = (list.sequences || []).find((s) => s.name === name);
  if (found) return found.id;
  const created = await kit("/sequences", { method: "POST", body: JSON.stringify({ name }) });
  return created.sequence.id;
}

async function main() {
  for (const seq of SEQUENCES) {
    const id = await ensureSequence(seq.name);
    console.log(`\n→ ${seq.name} (id ${id})`);
    const existing = await kit(`/sequences/${id}/emails?per_page=500`);
    const have = new Set((existing.emails || []).map((e) => (e.subject || "").trim()));
    for (const e of seq.emails) {
      if (have.has(e.subject.trim())) { console.log(`   · ${e.subject.slice(0, 44)} (exists)`); continue; }
      await kit(`/sequences/${id}/emails`, {
        method: "POST",
        body: JSON.stringify({
          subject: e.subject,
      ...
      console.log(`   ✅ +${e.delay}d  ${e.subject.slice(0, 44)}`);
    }
  }
  console.log("\nDone. Sequences + emails created (as drafts). Next:");
  console.log("  1) In Kit, open each sequence, review, and Publish each email + the sequence.");
  console.log("  2) Build the 2 automations from kit-build-guide.md (UI-only).");
}
